...
def test_validate_raw_valid_dict_passes(base_corpus, base_range_table):
    ...
    validator = OutputValidator(corpus=base_corpus, range_table=base_range_table, include_golden_check=False, reference_date=date(2024, 1, 1))
    raw = {
        "response": {
            "jurisdiction": "US-CA",
            "rate": 0.0925,
        ...
        "citations": [{"document_id": "IRS-2023-001"}, {"document_id": "CA-FTB-2023-TAX"}, {"document_id": "USC-TITLE26-SEC11"}],
...
def test_validate_raw_missing_required_field_fails(base_corpus, base_range_table):
    from llm_output_validator import OutputValidator
    validator = OutputValidator(corpus=base_corpus, range_table=base_range_table, include_golden_check=False)
    raw = {
        "response": {
            "rate": 0.09,
            "effective_date": "2023-07-01",
...
def test_validate_raw_returns_structured_errors(base_corpus, base_range_table):
    from llm_output_validator import OutputValidator
    validator = OutputValidator(corpus=base_corpus, range_table=base_range_table, include_golden_check=False)
    raw = {"response": {"rate": 2.0, "effective_date": "2023-01-01", "source_id": "x", "confidence": "high"}}
